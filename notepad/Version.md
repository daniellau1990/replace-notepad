...
- **Feature Step 2 增加设计探针**：领域建模后 5-10 行脚本验证 API 存在性、集成点、
  边界条件、失败预演。不等实现阶段才发现 API 不存在（v0.3.15–v0.3.19 教训）
...
- **Bug Fix**:
  1. 修复 CRLF 二重转换导致行间空行——4 处 `open()` 写入加 `newline=''`
  2. 启动时不再默认打开标签，改为完全空白窗口
  3. 右下角路径文字可自由选中复制，右键菜单新增"全选并复制"
  4. 路径修改弹窗改为双击触发（原单击与文本选择冲突）
- 工作流升级: 单轨→两轨制（Feature/Bug Fix），删除计划文件门禁 hook，集成 mattpocock 技能
